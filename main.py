...
def fetch_answer_async(text, direction_to, send_notification = None):
	url = base_url
	#
	post_fields = {'msg': text}  # Set POST fields here
...
def answer(text, type = "dota", send_notification = None):
	new_text = text
	if not text.strip():
	...
	try:
		...
		new_text = fetch_answer_async(text, type, send_notification=send_notification)
	...
	results = [
		...
		InlineQueryResultArticle(
			...
			input_message_content=InputTextMessageContent(
				answer(query, type="ods"), parse_mode=ParseMode.MARKDOWN),
			thumb_url=ods_thumb,
		),
		InlineQueryResultArticle(
			id=uuid4(),
			...
			input_message_content=InputTextMessageContent(
				answer(query, type="dota"), parse_mode=ParseMode.MARKDOWN),
